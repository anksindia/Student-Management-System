...
export class Dashboard implements OnInit {
  ...
  async saveStudent() {
    ...
    try {
      const res = await fetch(`${API_URL}/${this.editId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...this.form,
  ...
  // Event handlers for mouse over effects (if needed for custom button styling)
  onMouseOver(event: MouseEvent) {
    const target = event.target as HTMLElement | null;
    if (target) target.style.background = '#d9363e';
  }

  onMouseOut(event: MouseEvent) {
    const target = event.target as HTMLElement | null;
    if (target) target.style.background = '#ff4d4f';
  }
}
